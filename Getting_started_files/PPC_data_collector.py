    ...
    def construct_file_dict(self, read_data, read_data2, date, name, block_time = -1):
        ...
        if self.script_type == 'DarkIV':
            ...
            try:
                ...
                laser_wavl = "1550 nm"
                print("Exception generated by file: ",name)
                        ...
                        for curr in self.dictionary[name][pos][filt].keys():
                            if items == 'initial':
                                ...
                                try:
                                    ...
                                    V = np.array(self.dictionary[name][pos][filt][curr]['Voltage (V)'])
                                    # Estimate Isc
                                    Isc = I[np.where(V==0)[0][0]]
                                    # Estimate Voc
                                    i1 = np.argmin(I[I>0])
                                    i2 = np.argmax(I[I<0])
                                    I_fit = [I[I>0][i1], I[I<0][i2]]
                                    V_fit = [V[I>0][i1], V[I<0][i2]]
                                    Voc = np.polyfit(I_fit,V_fit,1)[1]
                                    # Calculate power
                                    P = -I*V
                                    # Estimate Pmax, Vmp, Imp
                                    Pmax = np.max(P)
                                    i = np.argmax(P)
                                    Vmp = V[i]
                                    Imp = I[i]
                                    # Calculate FF
                                    FF = -Pmax/(Isc*Voc)
                                    self.dictionary[name][pos][filt][curr].update({'Vmp (V)':Vmp})
                                    self.dictionary[name][pos][filt][curr].update({'Imp (A)':Imp})
                                    self.dictionary[name][pos][filt][curr].update({'Pmax (I)':abs(Vmp*Imp)})
                                    self.dictionary[name][pos][filt][curr].update({'FF':FF})
                                    self.dictionary[name][pos][filt][curr].update({'Voc (V)':Voc})
                                    self.dictionary[name][pos][filt][curr].update({'Isc (A)':Isc})
                                except KeyError as err:
                                    print('KeyError in add basic of add_to_dict:', err)
                                    print(name, pos, filt, curr)
                            #elif items == "Voc_Isc":
                            else:
