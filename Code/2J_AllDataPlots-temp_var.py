...
colours = (cycler(color=sns.color_palette('rocket',  n_colors = 48)))
#
...
plt.show()
#%% PLOT Responsivity VS IRRADIANCE####################################################
...
ax1.plot(x1, np.divide(y_prime,np.array(x1)*Atot), c = 'blue', marker = markers[i], ls = linestyles[i], lw = linewidth,
         label = "C5245-X7Y0 25$\degree$C")
x2, y2 = plt_PPC.grab_data(FixedVoltage_data.dictionary, 'C5245-X7Y0', ['NaN'], ['ND1 + ND2','ND2','ND1','No Filter'],['6.0 (1)','8.0 (1)','10.0 (1)','12.0 (1)','14.0 (1)','16.0 (1)','18.0 (1)', '6.5 (1)', '7.0 (1)'],
                      'Power (W)', 'Current (A)',
                      # pull out data only from a specific date (optional)
                      date =  dates[i],
                      # pull out data only with a specific wavelength (1550nm or 1310nm)
...
for y in y2:
    ...
    n += 1
#resp = np.divide(y2,x1)
ax1.plot(x2, np.divide(y_prime,np.array(x2)*Atot), 'violet', marker = markers[i], ls = linestyles[i], lw = linewidth,
         label = "C5245-X7Y0 23$\degree$C")
x3, y3 = plt_PPC.grab_data(FixedVoltage_data.dictionary, 'C5245-X7Y0', ['NaN'], ['ND1 + ND2','ND2','ND1','No Filter'],['6.0 (2)','8.0 (2)','10.0 (2)','12.0 (2)','14.0 (2)','16.0 (2)','18.0 (2)', '6.5 (2)', '7.0 (2)'],
                      'Power (W)', 'Current (A)',
                      # pull out data only from a specific date (optional)
                      date =  dates[i],
                      # pull out data only with a specific wavelength (1550nm or 1310nm)
...
for y in y3:
    ...
    n += 1
#resp = np.divide(y2,x1)
ax1.plot(x3, np.divide(y_prime,np.array(x3)*Atot), c = 'orange', marker = markers[i], ls = linestyles[i], lw = linewidth,
